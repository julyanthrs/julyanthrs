...
const padIndex = (index: number, length = 2): string => String(index).padStart(length, "0");
...
export function IntroSequence() {
  ...
  if (reducedMotion) {
    return (
      <section data-section="Intro" aria-label="What I do" className="introSeq-section relative z-20 space-y-10 py-32">
        {PHRASES.map((lines) => (
          <p key={lines.join(" ")} className="introSeq-display text-[14vw] text-white md:text-[9vw]">
            {lines.join(" ")}
          </p>
        ))}
        <p className="introSeq-serif max-w-2xl text-3xl leading-snug text-[#ffc1dc] md:text-4xl">{DESCRIPTION}</p>
      </section>
    );
  }
  ...
  return (
    <section
      ...
      className="introSeq-section relative z-20 h-[100svh] overflow-hidden"
    >
      <div className="introSeq-shell relative flex h-full flex-col justify-center">
        <p className="sr-only">
            ...
            <div key={phraseIndex} data-phrase={phraseIndex} className="absolute inset-x-0 top-0 will-change-transform">
              {lines.map((line, lineIndex) => (
                <span
                  key={line}
                  data-line
                  className={`introSeq-display block overflow-hidden pb-[0.06em] text-[17vw] md:text-[10.5vw] ${
                    lineIndex === 1 ? "ml-[8vw] text-[#ff2d95] md:ml-[16vw]" : "text-white"
                  } ${phraseIndex === 1 ? "introSeq-display-wide" : phraseIndex === 2 ? "introSeq-display-condensed" : ""}`}
                >
        ...
        <p
          data-description
          className="introSeq-serif relative ml-auto mt-10 max-w-xl text-2xl leading-snug text-[#ffc1dc] md:mr-[8vw] md:text-4xl"
          aria-hidden="true"
        >
        ...
        <div className="absolute inset-x-[var(--introseq-gutter)] bottom-8 flex items-center gap-4" aria-hidden="true">
          <span className="introSeq-annotation text-[#ff2d95]">
            <span ref={counterRef}>01</span> / 03
          </span>
          <span className="relative h-px flex-1 bg-white/10">
            <span data-intro-progress className="absolute inset-0 origin-left scale-x-0 bg-[#ff2d95]" />
          </span>
        </div>
      </div>
    </section>
